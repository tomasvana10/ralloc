...
export function GroupSessionViewer({
  ...
  const { data, joinGroup, leaveGroup, currentGroup, lock } = useGroupSession({
    ...
    onOpen: () => toast.info("You've been connected to the group session."),
    onClose: (code) => {
      ...
      if (code === GroupSessionS2C.CloseEventCodes.GroupSessionWasDeleted) {
        toast.error(
          "This group session was deleted. You'll be redirected in 3 seconds.",
        );
        lock();
      ...
      if (code === GroupSessionS2C.CloseEventCodes.RateLimited) {
        toast.error(
          "You're sending too many requests. Please try again soon. You'll be redirected in 3 seconds.",
        );
        lock();
        return setTimeout(() => router.push("/"), 3000);
      }
